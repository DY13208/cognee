...
function wrapper({ children }: { children: ReactNode }) {
  return createElement(NextIntlClientProvider, { locale: "en", messages: en }, children);
}
...
describe("useSurveyWidgetState", () => {
  ...
  it("starts on the score step with no score selected", () => {
    const { result } = renderHook(() => useSurveyWidgetState(RESPONSE_ID, jest.fn()), { wrapper });
  ...
  it("calls onDone when closed", () => {
    ...
    const { result } = renderHook(() => useSurveyWidgetState(RESPONSE_ID, onDone), { wrapper });
