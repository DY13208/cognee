...
export default function CreatePromptModal({
  ...
  return (
    <ModalShell width={440} onClose={() => { if (!inferringPrompt) onCancel(); }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: "#EDECEA", margin: 0 }}>{t("createPrompt.title")}</h2>
      {inferringPrompt ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "24px 0" }}>
          <Loader size={24} color="#6510F4" />
          <span style={{ fontSize: 14, color: "#6510F4", fontWeight: 500 }}>{t("createPrompt.generating", { modelName: modelName ?? t("createPrompt.fallbackModelName") })}</span>
          <span style={{ fontSize: 12, color: "rgba(237,236,234,0.55)" }}>{t("createPrompt.wait")}</span>
        </div>
      ) : (
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 14, fontWeight: 500, color: "#EDECEA" }}>{t("createPrompt.generateFromModel")}</span>
                <span style={{ fontSize: 12, color: "rgba(237,236,234,0.55)" }}>
                  {modelName ? t("createPrompt.usingModel", { modelName }) : t("createPrompt.selectModelFirst")}
                </span>
              </div>
            </button>
            <button
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <span style={{ fontSize: 14, fontWeight: 500, color: "#EDECEA" }}>{t("createPrompt.startBlank")}</span>
                <span style={{ fontSize: 12, color: "rgba(237,236,234,0.55)" }}>{t("createPrompt.startBlankHint")}</span>
              </div>
            </button>
          </div>
          <button
