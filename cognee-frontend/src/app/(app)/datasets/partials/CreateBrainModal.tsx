...
export default function CreateBrainModal({
  ...
  return (
    <ModalShell onClose={onCancel}>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: "#EDECEA", margin: 0 }}>{t("createBrain.title")}</h2>
      <p style={{ fontSize: 13, color: "rgba(237,236,234,0.55)", margin: 0 }}>{t("createBrain.description")}</p>
      <input
        autoFocus
        type="text"
        value={value}
        ...
        placeholder={t("createBrain.placeholder")}
      ...
      />
      {hasInvalidChars ? (
        <p style={{ fontSize: 13, color: "#EF4444", margin: 0 }}>{t("createBrain.periodError")}</p>
      ) : (
        <p style={{ fontSize: 12, color: "rgba(237,236,234,0.35)", margin: 0 }}>{t("createBrain.nameHint")}</p>
      )}
