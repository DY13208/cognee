...
function StatusBadge({ status }: { status: string }) {
  const t = useTranslations("integrations.connections");
  const color = status === "LIVE" ? "#22C55E" : status === "STAGING" ? "#F59E0B" : "#A1A1AA";
  const label = status === "LIVE" ? t("live") : status === "STAGING" ? t("staging") : t("inactive");
...
export default function ConnectionsPage() {
  ...
  const [createAgentError, setCreateAgentError] = useState(false);
  ...
  const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
    { id: "agents", label: t("tabAgents"), icon: <AgentIconSmall /> },
    { id: "my-datasets", label: t("tabMyDatasets"), icon: <PersonIcon /> },
    { id: "org-shared", label: t("tabOrgShared"), icon: <OrgIcon /> },
  ];
    ...
    <div style={{ padding: 32, display: "flex", flexDirection: "column", gap: 24, fontFamily: '"Inter", system-ui, sans-serif', height: "100%" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          <h1 style={{ fontSize: 20, fontWeight: 300, color: "#18181B", margin: 0, fontFamily: '"TWK Lausanne", system-ui, sans-serif' }}>{t("title")}</h1>
          <span style={{ fontSize: 14, color: "#71717A" }}>{t("description")}</span>
        </div>
        <Link
          href="#"
          onClick={(event) => {
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #E4E4E7" }}>
        {TABS.map((tabItem) => (
          <button
            key={tabItem.id}
            onClick={() => { trackEvent({ pageName: "Connections", eventName: "connections_tab_switched", additionalProperties: { tab: tabItem.id } }); setTab(tabItem.id); }}
            className="cursor-pointer"
            style={{
              display: "flex", alignItems: "center", gap: 6,
              padding: "10px 20px",
              background: "none", border: "none", borderBottom: tab === tabItem.id ? "2px solid #6510F4" : "2px solid transparent",
              fontSize: 14, fontWeight: tab === tabItem.id ? 500 : 400,
              color: tab === tabItem.id ? "#6510F4" : "#71717A",
              fontFamily: "inherit", marginBottom: -1,
            }}
          >
            <span style={{ color: tab === tabItem.id ? "#6510F4" : "#A1A1AA" }}>{tabItem.icon}</span>
            {tabItem.label}
          </button>
        ))}
      </div>
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowShareModal(false)}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: 24, width: 480, maxHeight: "80vh", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
              <h2 style={{ fontSize: 18, fontWeight: 600, color: "#18181B", margin: 0 }}>{t("manageAccess")}</h2>
              <p style={{ fontSize: 13, color: "#71717A", margin: 0 }}>{t.rich("shareWith", { name: selectedAgent.agent_type, strong: (chunks) => <strong>{chunks}</strong> })}</p>

              {/* Shared datasets */}
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: 0.3 }}>{t("hasAccess", { count: sharedDs.length })}</span>
                <div style={{ border: "1px solid #E4E4E7", borderRadius: 8, overflow: "hidden" }}>
                  {sharedDs.length === 0 ? (
                    <div style={{ padding: "14px 12px", textAlign: "center", fontSize: 13, color: "#A1A1AA" }}>{t("noSharedYet")}</div>
                  ) : (
                    <div style={{ maxHeight: 180, overflowY: "auto" }}>
                      {sharedDs.map((d) => (
                        <div key={d.id} style={{ display: "flex", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #F4F4F5", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 11, color: "#A1A1AA", flexShrink: 0 }}>{d.ownerId === selectedAgent.id ? t("owned") : t("read")}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {/* Unshared datasets */}
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: "#71717A", textTransform: "uppercase", letterSpacing: 0.3 }}>{t("noAccessCount", { count: unsharedDs.length })}</span>
                <div style={{ border: "1px solid #E4E4E7", borderRadius: 8, overflow: "hidden" }}>
                  {unsharedDs.length === 0 ? (
                    <div style={{ padding: "14px 12px", textAlign: "center", fontSize: 13, color: "#A1A1AA" }}>{t("allShared")}</div>
                  ) : (
                    <div style={{ maxHeight: 180, overflowY: "auto" }}>
                      {unsharedDs.map((d) => (
                        <div key={d.id} style={{ display: "flex", alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #F4F4F5", gap: 8 }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <button onClick={() => setShowShareModal(false)} className="cursor-pointer" style={{ background: "#6510F4", border: "none", borderRadius: 8, padding: "8px 20px", fontSize: 13, fontWeight: 500, color: "#fff", fontFamily: "inherit" }}>{t("done")}</button>
              </div>
            </div>
          </div>
        );
      ...
      {showCreateAgentModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={closeCreateAgentModal}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: 24, width: 480, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
            {!createdAgent ? (
              <>
                <h2 style={{ fontSize: 18, fontWeight: 600, color: "#18181B", margin: 0 }}>{t("createAgent")}</h2>
                ...
                />
                {createAgentError && (
                  <span style={{ fontSize: 12, color: "#DC2626" }}>{t("createFailed")}</span>
                )}
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button onClick={closeCreateAgentModal} className="cursor-pointer" style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 500, color: "#3F3F46", fontFamily: "inherit" }}>{common("cancel")}</button>
                  <button
              ...
              <>
                <h2 style={{ fontSize: 18, fontWeight: 600, color: "#18181B", margin: 0 }}>{t("createdTitle")}</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 500, color: "#71717A" }}>{t("agentId")}</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 500, color: "#71717A" }}>{t("agentEmail")}</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 500, color: "#71717A" }}>{t("apiKey")}</span>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#F4F4F5", borderRadius: 6, padding: "8px 12px" }}>
                      ...
                      >
                        {copiedKey ? common("copied") : common("copy")}
                      </button>
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", justifyContent: "flex-end" }}>
                  <button onClick={closeCreateAgentModal} className="cursor-pointer" style={{ background: "#6510F4", border: "none", borderRadius: 8, padding: "8px 20px", fontSize: 13, fontWeight: 500, color: "#fff", fontFamily: "inherit" }}>{t("done")}</button>
                </div>
              </>
            )}
          </div>
      ...
      {showDeleteAgentModal && selectedAgent && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => !deletingAgent && setShowDeleteAgentModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: 24, width: 420, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 16px 48px rgba(0,0,0,0.12)" }}>
            <h2 style={{ fontSize: 18, fontWeight: 600, color: "#18181B", margin: 0 }}>{t("deleteAgent")}</h2>
            <p style={{ fontSize: 13, color: "#71717A", margin: 0, lineHeight: "20px" }}>
              {t.rich("deleteConfirm", { name: selectedAgent.agent_type, strong: (chunks) => <strong>{chunks}</strong> })}
            </p>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button
              ...
              >
                {deletingAgent ? t("deleting") : common("delete")}
              </button>
            </div>
          </div>
        </div>
      ...
      {tab === "agents" && agentUsers.length === 0 && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: "60px 20px", flex: 1 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 500, color: "#18181B" }}>{t("emptyTitle")}</span>
          <span style={{ fontSize: 13, color: "#71717A", textAlign: "center", maxWidth: 360 }}>{t("emptyDescription")}</span>
        ...
        <div style={{ display: "flex", gap: 0, flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, paddingLeft: 24, display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
            {selectedAgent ? (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 13, color: "#71717A" }}>
                      {t("datasetCount", { count: agentDatasets.length })}
                      {" · "}
                      {selectedAgent.last_active ? t("lastActive", { time: timeAgo(selectedAgent.last_active, locale) }) : t("neverConnected")}
                    </span>
                  </div>
                ...
                <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, overflow: "hidden" }}>
                  <div style={{ display: "flex", padding: "10px 16px", borderBottom: "1px solid #E4E4E7" }}>
                    <span style={{ flex: 1, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", minWidth: 0 }}>{t("columnDataset")}</span>
                    <span style={{ width: 80, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnOwner")}</span>
                    <span style={{ width: 90, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnAccess")}</span>
                    <span style={{ width: 70, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnStatus")}</span>
                  </div>
                  {agentDatasets.length === 0 ? (
                    <div style={{ padding: "20px 16px", textAlign: "center" }}>
                      <span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("noDatasetsSharedHint")}</span>
                    </div>
                  ) : (
                    agentDatasets.map((d, i) => {
                      ...
                      <div key={d.id} style={{ display: "flex", alignItems: "center", padding: "10px 16px", borderBottom: i < agentDatasets.length - 1 ? "1px solid #F4F4F5" : "none" }}>
                        ...
                        <span style={{ width: 80, fontSize: 12, color: "#3F3F46", flexShrink: 0 }}>{isOwned ? t("ownerAgent") : t("ownerShared")}</span>
                        <span style={{ width: 90, fontSize: 12, color: "#3F3F46", flexShrink: 0 }}>{isOwned ? t("readWrite") : (() => { const p = agentPerms[d.id]; if (!p) return t("read"); const hasRead = p.has("read"); const hasWrite = p.has("write"); if (hasRead && hasWrite) return t("readWrite"); if (hasWrite) return t("write"); return t("read"); })()}</span>
                        <span style={{ width: 70, fontSize: 12, color: "#22C55E", fontWeight: 500, flexShrink: 0 }}>{t("indexed")}</span>
                      </div>
                      );
                    })
                  )}
              ...
              <span style={{ fontSize: 14, color: "#A1A1AA", padding: 20 }}>{t("selectAgent")}</span>
            )}
          </div>
        </div>
      )}
      ...
      {tab === "my-datasets" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: "#18181B" }}>{t("myDatasetsTitle")}</span>
            <span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("myDatasetsHint")}</span>
          </div>
          <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ display: "flex", padding: "12px 20px", borderBottom: "1px solid #E4E4E7" }}>
              <span style={{ flex: 1, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase" }}>{t("columnName")}</span>
              <span style={{ width: 120, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnStatus")}</span>
              <span style={{ width: 100, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnShared")}</span>
            </div>
            {myDatasets.map((d, i) => (
              <div key={d.id} style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: i < myDatasets.length - 1 ? "1px solid #F4F4F5" : "none" }}>
                ...
                <span style={{ width: 120, fontSize: 13, color: "#22C55E", fontWeight: 500, flexShrink: 0 }}>{t("indexed")}</span>
                <span style={{ width: 100, flexShrink: 0 }}>
                  {(() => {
                    ...
                    return <HoverTooltip label={`${count} ${t("tabAgents")}`} items={s.agents} />;
                  })()}
                </span>
              </div>
            ))}
            {myDatasets.length === 0 && (
              <div style={{ padding: "24px 20px", textAlign: "center" }}><span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("noPersonal")}</span></div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === "org-shared" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: "#18181B" }}>{t("sharedTitle")}</span>
            <span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("sharedHint")}</span>
          </div>
          <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ display: "flex", padding: "10px 16px", borderBottom: "1px solid #E4E4E7" }}>
              <span style={{ flex: 1, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", minWidth: 0 }}>{t("columnName")}</span>
              <span style={{ width: 160, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("usedBy")}</span>
              <span style={{ width: 70, fontSize: 11, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnStatus")}</span>
            </div>
            {datasets.map((d, i) => {
              const s = datasetSharedWith[d.id];
              ...
              const parts: string[] = [];
              if (userCount > 0) parts.push(t("userCount", { count: userCount }));
              if (agentCount > 0) parts.push(`${agentCount} ${t("tabAgents")}`);
            ...
            {datasets.length === 0 && (
              <div style={{ padding: "24px 20px", textAlign: "center" }}><span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("noSharedDatasets")}</span></div>
            )}
          </div>
        </div>
      )}
...
function AgentMetrics({ sessions }: { sessions: SessionRow[] }) {
  ...
  const metrics = [
    { label: t("sessions"), value: String(total), color: "#18181B" },
    { label: t("successRate"), value: `${successRate.toFixed(1)}%`, color: successRate >= 95 ? "#16A34A" : successRate >= 80 ? "#D97706" : "#DC2626" },
    { label: t("tokens"), value: fmt(totalTokens), color: "#18181B" },
    { label: t("errors"), value: String(totalErrors), color: totalErrors > 0 ? "#DC2626" : "#18181B" },
  ];
...
function AgentSessionHistory({ sessions }: { sessions: SessionRow[] }) {
  ...
  if (visible.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <span style={{ fontSize: 14, fontWeight: 600, color: "#18181B" }}>{t("recentSessions")}</span>
        <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, padding: "24px 20px", textAlign: "center" }}>
          <span style={{ fontSize: 13, color: "#A1A1AA" }}>{t("noSessionsYet")}</span>
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #E4E4E7", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ display: "flex", padding: "10px 20px", borderBottom: "1px solid #E4E4E7" }}>
          <span style={{ width: 100, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnStatus")}</span>
          <span style={{ flex: 1, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase" }}>{t("columnSession")}</span>
          <span style={{ width: 100, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0 }}>{t("columnModel")}</span>
          <span style={{ width: 80, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0, textAlign: "right" }}>{t("tokens")}</span>
          <span style={{ width: 80, fontSize: 12, fontWeight: 500, letterSpacing: "0.5px", color: "#71717A", textTransform: "uppercase", flexShrink: 0, textAlign: "right" }}>{t("columnTime")}</span>
        </div>
        {visible.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ width: 80, fontSize: 12, color: "#A1A1AA", flexShrink: 0, textAlign: "right" }}>
                {s.last_activity_at ? timeAgo(s.last_activity_at, locale) : "—"}
              </span>
            </Link>
          );
        })}
