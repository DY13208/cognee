  ...
  const handleFiles = (newFiles: FileList | File[]) => {
    ...
    setFiles((prev) => {
      ...
      if (combined.length > MAX_FILES_PER_UPLOAD) {
        setLimitError(t("data.fileLimit", { max: MAX_FILES_PER_UPLOAD }));
      ...
      <div style={{ alignItems: "center", display: "flex", flexDirection: "column", paddingBottom: 40, paddingLeft: "clamp(16px, 5vw, 80px)", paddingRight: "clamp(16px, 5vw, 80px)", paddingTop: 48, width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ alignItems: "center", display: "flex", flexDirection: "column", gap: 8, paddingBottom: 40 }}>
          <div style={{ background: "rgba(188,155,255,0.20)", borderRadius: 100, border: "1px solid rgba(188,155,255,0.35)", padding: "5px 12px" }}>
            <div style={{ color: "#EDECEA", fontSize: 13, lineHeight: "16px" }}>{t("step", { step: 1, total: 3 })}</div>
          </div>
          <div style={{ color: "#EDECEA", fontSize: 28, fontWeight: 300, lineHeight: "34px", paddingTop: 8, letterSpacing: "-0.02em", fontFamily: '"TWKLausanne", sans-serif' }}>{t("data.title")}</div>
          <div style={{ color: "rgba(237,236,234,0.65)", fontSize: 15, lineHeight: "22px", textAlign: "center", maxWidth: 440 }}>{t("data.description")}</div>
        </div>
        ...
        <div style={{ maxWidth: 880, width: "100%", background: "#2a2a2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.5)", display: "flex", flexDirection: "column", gap: 20, padding: "48px 64px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <div style={{ color: "#EDECEA", fontSize: 17, lineHeight: "22px" }}>{t("data.add")}</div>
            <div style={{ color: "rgba(237,236,234,0.65)", fontSize: 13, lineHeight: "16px" }}>{t("data.addDescription")}</div>
          </div>
          ...
          >
            ...
            <div style={{ color: "#BC9BFF", fontSize: 14, lineHeight: "18px" }}>{t("data.drop")}</div>
          ...
          {!showPaste ? (
            <div onClick={() => setShowPaste(true)} style={{ alignItems: "center", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, cursor: "pointer", display: "flex", gap: 8, paddingBlock: 12, paddingInline: 16 }}>
              ...
              <div style={{ color: "rgba(237,236,234,0.7)", flexShrink: 0, fontSize: 13, lineHeight: "16px" }}>{t("data.paste")}</div>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              <textarea
              ...
              <div className="flex gap-2">
                <button onClick={handlePasteSubmit} className="cursor-pointer" style={{ background: "#BC9BFF", border: "none", borderRadius: 8, color: "#1e1e1c", fontSize: 13, padding: "6px 16px" }}>{t("data.addText")}</button>
                <button onClick={() => { setShowPaste(false); setPasteText(""); }} className="cursor-pointer" style={{ background: "none", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "rgba(237,236,234,0.6)", fontSize: 13, padding: "6px 16px" }}>{t("data.cancel")}</button>
              </div>
            </div>
          )}
        </div>
        ...
        {files.length > 0 && (
          <div style={{ paddingTop: 24 }}>
            <button onClick={() => { track({ pageName: "Onboarding", eventName: "onboarding_step_completed", additionalProperties: { step: "1", file_count: String(files.length) } }); onNext(); }} className="cursor-pointer" style={{ background: "#BC9BFF", border: "none", borderRadius: 8, padding: "11px 32px", fontSize: 14, fontWeight: 500, color: "#1e1e1c", letterSpacing: "-0.01em" }}>
              {t("data.continueWith", { count: files.length })}
            </button>
          </div>
        )}
