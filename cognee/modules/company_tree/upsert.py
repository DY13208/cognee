...
async def get_company_tree(
    ...
    # Keep the dataset DB context open for the whole read — exiting early
    # releases the queue slot and can race with concurrent graph ops.
...
async def upsert_company_tree(
    ...
    async with set_database_global_context_variables(dataset_id, dataset.owner_id):
        ...
        room_of = {
            item.source_uid: item.source_room or payload.source_room for item in payload.nodes
        }
